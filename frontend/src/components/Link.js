...
const Link = (props) => {
    ...
    const Renderlink = (props) => {
        ...
        } else if (link.linkType === "Normal Image Link" || link.linkType === "Drive Image Link") {
        ...
        <>
            <div className='' style={mystyle.container(isRowBased)} >
                <div className={'d-flex justify-content-between p-2 link animation'+link.animation }  >
                    ...
                    <div className='m-2'>
                        {link.linkType === "Normal Link" ? <a href={link.link} target='_blank' onClick={() => clickInfo(link._id, link.link)} style={{ textDecoration: 'none', color: 'black' }}><h6 style={{ marginBottom: "0" }}>{link.description}</h6></a> : <h6 style={{ marginBottom: "0" }}>{link.description}</h6>}
                    </div>
                    <div className='d-flex justify-content-center align-items-center' style={{ width: "40px", height: "40px", borderRadius: "100%" }} >
                        {link.linkType === 'Normal Link' ? "" : <button onClick={() => { setShown(!shown); shown && clickInfo(link._id, link.link) }} style={{ border: 'none', backgroundColor: 'transparent' }}>{shown ? <Icon.ChevronUp size={16} /> : <Icon.ChevronDown size={16} />}</button>}
                    </div>
